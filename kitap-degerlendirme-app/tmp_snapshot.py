...
CASES = [
    {'title': 'Tavşan Pati', 'path': r'uploads\\arkadaslik_oykuleri_tavsan_patinin_sasirtici_yolculugu_ic.pdf', 'author': 'Ozlem Aytek'},
    {'title': 'Büyülü Yastıklar', 'path': r'uploads\\buyulu_yastiklar.pdf', 'author': 'Test'},
    {'title': 'Benim Adım Kristof Kolomb', 'path': r'uploads\\benim_adim_kristof_kolomb.pdf', 'author': 'Test'},
    {'title': 'Gökyüzünü Kaybeden Şehir', 'path': r'uploads\\gokyuzunu_kaybeden_sehir.pdf', 'author': 'Test'},
]
...
def load_latest_payload():
    for name in RUNTIME_NAMES:
        if os.path.exists(name):
            try:
                return json.loads(open(name, 'r', encoding='utf-8').read())
            except Exception:
                continue
    return {}
...
def compute_snapshot(payload):
    got = {
        ...
        'summary_keys_present': sorted([k for k in ['canonical_summary', 'summary_ui', 'summary_pdf', 'teacher_summary', 'summary_before_gate', 'summary_after_gate', 'summary_rendered_to_ui', 'summary_used_for_pdf'] if k in payload]),
    }
    values = [got['summary_ui_hash'], got['summary_pdf_hash'], got['teacher_summary_hash'], got['canonical_summary_ir_hash']]
