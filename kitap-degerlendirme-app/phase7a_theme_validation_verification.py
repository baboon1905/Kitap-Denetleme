#!/usr/bin/env python3
"""
Phase 7A Narrative-Aware Theme Validation Verification
"""
import copy
import io
import json
...
def run_for_book(title: str):
    ...
    if isinstance(narrative, dict):
        validation_checks = {
            "theme_validation_present": "theme_validation" in narrative,
            "theme_validation_only_in_shadow": True,
            "theme_validation_item_count": len(narrative.get("theme_validation") or []),
            "validation_diagnostics_present": isinstance(narrative.get("diagnostics"), dict),
        }

    production_payload = true_payload
    production_leakage = {
        "root_has_theme_validation": "theme_validation" in production_payload,
        "shadow_only_structure_present": isinstance(true_shadow, dict) and isinstance(narrative, dict) and "theme_validation" in narrative,
        "production_payload_leaks_detected": "theme_validation" in production_payload,
    }
...
if __name__ == '__main__':
    ...
    for book in books:
        ...
        book_outpath = os.path.join(os.path.dirname(__file__), f"phase7a_theme_validation_verification_{book.replace(' ', '_')}.json")
        with open(book_outpath, 'w', encoding='utf-8') as fh:
            json.dump(book_result, fh, ensure_ascii=False, indent=2)
    final = {"books": results}
    outpath = os.path.join(os.path.dirname(__file__), "phase7a_theme_validation_verification.json")
    with open(outpath, 'w', encoding='utf-8') as fh:
        json.dump(final, fh, ensure_ascii=False, indent=2)
    print(json.dumps({"result_file": outpath, "book_result_files": [os.path.join(os.path.dirname(__file__), f"phase7a_theme_validation_verification_{book.replace(' ', '_')}.json") for book in books]}, ensure_ascii=False))
