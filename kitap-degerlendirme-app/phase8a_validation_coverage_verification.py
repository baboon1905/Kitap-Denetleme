#!/usr/bin/env python3
"""
Phase 8A Validation Coverage Verification
"""
import copy
import io
import json
...
def run_for_book(title: str):
    ...
    validation_coverage = narrative.get("validation_coverage") if isinstance(narrative, dict) else None
    ...
    return {
        ...
        "compare": cmp,
        "shadow_only_check": {
            "validation_coverage_only_in_shadow": isinstance(validation_coverage, dict),
            "validation_coverage_in_narrative_only": isinstance(true_shadow, dict) and isinstance(narrative, dict) and isinstance(validation_coverage, dict),
            "production_payload_has_validation_coverage": "validation_coverage" in true_payload,
            "production_payload_has_validation_diagnostics": "theme_validation_coverage" in true_payload,
        },
...
if __name__ == '__main__':
    ...
    for book in books:
        ...
        book_outpath = os.path.join(os.path.dirname(__file__), f"phase8a_validation_coverage_verification_{book.replace(' ', '_')}.json")
        with open(book_outpath, 'w', encoding='utf-8') as fh:
            json.dump(book_result, fh, ensure_ascii=False, indent=2)
    final = {"books": results}
    outpath = os.path.join(os.path.dirname(__file__), "phase8a_validation_coverage_verification.json")
    with open(outpath, 'w', encoding='utf-8') as fh:
        json.dump(final, fh, ensure_ascii=False, indent=2)
    print(json.dumps({"result_file": outpath, "book_result_files": [os.path.join(os.path.dirname(__file__), f"phase8a_validation_coverage_verification_{book.replace(' ', '_')}.json") for book in books]}, ensure_ascii=False))
