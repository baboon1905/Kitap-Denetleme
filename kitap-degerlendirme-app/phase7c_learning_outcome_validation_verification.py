#!/usr/bin/env python3
"""
Phase 7C Narrative-Aware Learning Outcome Validation Verification
"""
import copy
import io
import json
...
def run_for_book(title: str):
    ...
    learning_outcome_validation_checks = {}
    if isinstance(narrative, dict):
        learning_outcome_validation_checks = {
            'learning_outcome_validation_present': 'learning_outcome_validation' in narrative,
            'learning_outcome_validation_only_in_shadow': True,
            'learning_outcome_validation_item_count': len(narrative.get('learning_outcome_validation') or []),
            'learning_outcome_validation_diagnostics_present': isinstance(narrative.get('diagnostics'), dict),
        }

    production_payload = true_payload
    production_leakage = {
        'root_has_learning_outcome_validation': 'learning_outcome_validation' in production_payload,
        'shadow_only_structure_present': isinstance(true_shadow, dict) and isinstance(narrative, dict) and 'learning_outcome_validation' in narrative,
        'production_payload_leaks_detected': 'learning_outcome_validation' in production_payload,
    }

    return {
        'title': title,
        'compare': compare_payloads(results.get('false'), results.get('true')),
        'learning_outcome_validation_checks': learning_outcome_validation_checks,
...
if __name__ == '__main__':
    ...
    for book in books:
        ...
        book_outpath = os.path.join(os.path.dirname(__file__), f"phase7c_learning_outcome_validation_verification_{book.replace(' ', '_')}.json")
        with open(book_outpath, 'w', encoding='utf-8') as fh:
            json.dump(book_result, fh, ensure_ascii=False, indent=2)
    final = {"books": results}
    outpath = os.path.join(os.path.dirname(__file__), "phase7c_learning_outcome_validation_verification.json")
    with open(outpath, 'w', encoding='utf-8') as fh:
        json.dump(final, fh, ensure_ascii=False, indent=2)
    print(json.dumps({"result_file": outpath, "book_result_files": [os.path.join(os.path.dirname(__file__), f"phase7c_learning_outcome_validation_verification_{book.replace(' ', '_')}.json") for book in books]}, ensure_ascii=False))
