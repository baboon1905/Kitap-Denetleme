#!/usr/bin/env python3
"""
Phase 7B Narrative-Aware Character Validation Verification
"""
import copy
import io
import json
...
def run_for_book(title: str):
    ...
    character_validation_checks = {}
    if isinstance(narrative, dict):
        character_validation_checks = {
            'character_validation_present': 'character_validation' in narrative,
            'character_validation_only_in_shadow': True,
            'character_validation_item_count': len(narrative.get('character_validation') or []),
            'character_validation_diagnostics_present': isinstance(narrative.get('diagnostics'), dict),
        }

    production_payload = true_payload
    production_leakage = {
        'root_has_character_validation': 'character_validation' in production_payload,
        'shadow_only_structure_present': isinstance(true_shadow, dict) and isinstance(narrative, dict) and 'character_validation' in narrative,
        'production_payload_leaks_detected': 'character_validation' in production_payload,
    }

    return {
        'title': title,
        'compare': compare_payloads(results.get('false'), results.get('true')),
        'character_validation_checks': character_validation_checks,
...
if __name__ == '__main__':
    ...
    for book in books:
        ...
        book_outpath = os.path.join(os.path.dirname(__file__), f"phase7b_character_validation_verification_{book.replace(' ', '_')}.json")
        with open(book_outpath, 'w', encoding='utf-8') as fh:
            json.dump(book_result, fh, ensure_ascii=False, indent=2)
    final = {"books": results}
    outpath = os.path.join(os.path.dirname(__file__), "phase7b_character_validation_verification.json")
    with open(outpath, 'w', encoding='utf-8') as fh:
        json.dump(final, fh, ensure_ascii=False, indent=2)
    print(json.dumps({"result_file": outpath, "book_result_files": [os.path.join(os.path.dirname(__file__), f"phase7b_character_validation_verification_{book.replace(' ', '_')}.json") for book in books]}, ensure_ascii=False))
